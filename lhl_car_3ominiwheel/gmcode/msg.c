#include "msg.h"
#define SEND_COMMAND_MIN_LENGTH 17
...
#define CHECK_OFFSET 15   // 校验位位置
#define FRAME_TAIL 16         // 帧尾位
//头都是0xAA,尾巴有区别
...
//c若原数值的小数部分 < 0.5，加 0.5 后不会进位（如 398.4 + 0.5 = 398.9，截断后为 398）。
    ...
    // 线速度
    data = (int16_t)(odom3_def.vx_speed_ * 1000 + 0.5f);
    memcpy(&(send_buff[VX_OFFSET]), &data, 2);
    data = (int16_t)(odom3_def.vy_speed_ * 1000 + 0.5f);
    memcpy(&(send_buff[VY_OFFSET]), &data, 2);

    // 角速度
    data = (int16_t)(odom3_def.angle_speed_ * 1000 + 0.5f);
    memcpy(&(send_buff[W_OFFSET]), &data, 2);

    // x位置
    data = (int16_t)(odom3_def.x_ * 1000 + 0.5f);
    memcpy(&(send_buff[X_POSITION]), &data, 2);

    // y位置
    data = (int16_t)(odom3_def.y_ * 1000 + 0.5f);
    memcpy(&(send_buff[Y_POSITION]), &data, 2);

    // 当前角度（弧度）
    data = (int16_t)(odom3_def.angle_ * 1000 + 0.5f);
...
//serial_printf("\r\n");  // 修正：/ → \
    ...
    HAL_UART_Transmit_DMA(&huart1, (uint8_t*)send_buff, 17);
}
...
CONTROL use_msg(uint8_t *command) {
    ...
    if (check_bit == *(command + RECHECK_OFFSET)) {
        if (*(command + RECOMMAND_MIN_LENGTH - 1) == 0x01U) {
            ...
            inverse_kinematic_3wheel(x_speed, y_speed, angular_speed, &omini3_car.wheel1_v, &omini3_car.wheel2_v, &omini3_car.wheel3_v);

            Set_PID_TargetSpeed(get_targetpulse(omini3_car.wheel1_v), 1);
            Set_PID_TargetSpeed(get_targetpulse(omini3_car.wheel2_v), 4);
            Set_PID_TargetSpeed(get_targetpulse(omini3_car.wheel3_v), 2);
            //  Set_PID_TargetSpeed(get_targetpulse(omini_car.beside_v), 4);
...
//}
            *(command + RECOMMAND_MIN_LENGTH - 1) = 0;//因为这是全局，不搞掉一直存在
            ...
            float x_speed = tf_speed(command, REVX_OFFSET) / 1000.0f;
            float y_speed = tf_speed(command, REVY_OFFSET) / 1000.0f;
            float angular_speed = tf_speed(command, REW_OFFSET) / 1000.0f;
            inverse_kinematic_3wheel(x_speed, y_speed, angular_speed, &omini3_car.wheel1_v, &omini3_car.wheel2_v, &omini3_car.wheel3_v);
