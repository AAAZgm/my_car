#include "motor.h"
 ...
 //要对m1的读取改极性，驱动和m3反
 //不懂就看原理图
  //m2 pwm用的timer1 ch2 ch1（pe11,9)，读取用tim2定(pa15,pb3),左，1，编码顺，驱动逆
 //m4 pwm用的timer10，11，ch3ch4(pb8,pb9),读取用
PID L_AddPID;
PID R_AddPID;
...
HAL_TIM_Encoder_Start(&htim5,TIM_CHANNEL_ALL);
    ...
    HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_4); 
	 	...
	 	HAL_TIM_PWM_Start(&htim1, TIM_CHANNEL_1);  //前二
    ...
    HAL_TIM_PWM_Start(&htim9, TIM_CHANNEL_2); 
			...
			HAL_TIM_PWM_Init(&htim10);		//前二
	 //__HAL_TIM_MOE_ENABLE(&htim10);//1，8需要
...
int16_t L_Encoder_Speed,R_Encoder_Speed;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
 ...
 //可以加个滤波但没必要
  ...
  if(htim == (&htim2))
  {
		static int16_t L_Output_Val=0,R_Output_Val=0; 
    //  设置测量频率为20Hz(50ms)
  //  uint8_t rate = 20;
